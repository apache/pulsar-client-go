...
func (p *partitionProducer) internalSend(sr *sendRequest) {
	...
	//	previous sendRequest has been split into chunk sendRequests,
	//	so we can recycle its resource here
	*sr = sendRequest{}
	sendRequestPool.Put(sr)
}
...
func (p *partitionProducer) updateChunkInfo(sr *sendRequest) error {
	...
	// Since a single message first generates all chunks and then acquires the semaphore at once,
	// we need to ensure the number of chunks generated by a single message is less than the size of the pendingQueue.
	// Otherwise, the current producer will be blocked when acquiring the semaphore.
	if sr.totalChunks > p.publishSemaphore.MaxPermits() {
		p.log.WithError(ErrMessageTooLarge).
			WithField("totalChunks", sr.totalChunks).
			WithField("pendingQueueSize", p.publishSemaphore.MaxPermits()).
			WithField("properties", sr.msg.Properties).
			Errorf(
				"Single message's chunk size: %d, exceeds pending queue max size: %d",
				sr.totalChunks, p.publishSemaphore.MaxPermits(),
			)
		return ErrMessageTooLarge
	}
	return nil
